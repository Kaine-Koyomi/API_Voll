...
public class MedicoController {
    ...
    @PostMapping
    @Transactional
    public ResponseEntity cadastro(@RequestBody @Valid DadosCadastroMedico json, UriComponentsBuilder uriBuilder){
    ...
    public ResponseEntity<Page<DadosListagemMedico>> listar(@PageableDefault(size = 10, sort = {"nome"}) Pageable pagination){
        var page = repository.findAllByActiveTrue(pagination).map(DadosListagemMedico:: new);
